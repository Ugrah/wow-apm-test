...
$(document).ready(function () {
    ...
    // Swiper user type sidebar
    var swiper = new Swiper('.swiper-sidebar-user-type', {
        slidesPerView: 'auto',
        spaceBetween: 0,
        pagination: false,
    });
    $('a.user-action').click(function(e) {
        e.preventDefault();
        var url = $(this).data('url');
        var form = $(`<form action="${url}" method="post">
            <input type="hidden" name="user_type" value="${$(this).data('type')}" />
            <input type="hidden" name="terminal_id" value="${$('select#terminals').val()}" />
        </form>`);
        $('body').append(form);
        form.submit();
    })
...
var sendFormWowSelector = document.querySelector("#send-form") ;
if(sendFormWowSelector){

  sendFormWowSelector.addEventListener("submit", function(e){
      e.preventDefault();
      var thisForm = this;
      AjaxInitialier({
          frm: JSON.stringify({
